...
#		text( x=0, -log10(p.sig) + 0.1, paste("p=",sig.level, sep=""), cex=0.6, srt=90, adj=c(0.5, -1));
	...
	points( 1:NROW(res), -log10(res[,3]), pch=20, col=cols[ as.numeric(res[,1])%%7+1 ], cex=0.5 );

	nLen <- table(as.character(res[,1]));
...
fpt.plot_manhattan_reduce_pdfsize<-function( res, p.05=NA, p.01=NA, sig.level=NA, p.sig=NA, map.title="" )
{
	op <- par( mgp = c( 1.5, 0.5,0 ), mar=c( 4, 4, 1.5, 1.5) );
...
#	points( 1:NROW(res), -log10(res[,3]), pch=20, col=cols[ (res[,1]%%7+1)], cex=0.5 );
	...
	points( idx1, -log10(res[idx1,3]), pch=20, col=cols[ as.numeric(res[idx1,1])%%7+1 ], cex=0.5 );
...
fpt.plot_curve <- function( obj.phe, par_h0=NULL, par_h1=NULL, snp.vec=NULL, include.rawdata=TRUE, include.meanvector=TRUE, extra=list(), ...)
{
	...
	{
	    if( !is.null(extra$METHOD) && !is.null(extra$NAME) && !is.null(extra$LR2) )
